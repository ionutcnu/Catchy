...
export default function OptionsApp() {
  ...
  // Load ignored errors from all active tabs
  const loadIgnoredErrors = async () => {
    try {
      // Get all tabs
      const tabs = await chrome.tabs.query({});
      const allIgnores: Set<string> = new Set();

      // Collect ignored errors from each tab
        ...
        try {
          const results = await chrome.scripting.executeScript({
            ...
            func: () => {
              ...
              if (browserIgnores) {
                try {
                  return JSON.parse(browserIgnores) as string[];
                } catch {
                  return [];
                }
              }
          ...
          if (results && results[0]?.result) {
            const tabIgnores = results[0].result as string[];
            tabIgnores.forEach((signature) => allIgnores.add(signature));
          }
        } catch {
      ...
      setIgnoredErrors(Array.from(allIgnores));
    } catch (error) {
      console.error('[Catchy Options] Failed to load ignored errors:', error);
    }
  };

  // Remove a specific ignored error from all tabs
  const handleRemoveIgnoredError = async (signature: string) => {
    try {
      const tabs = await chrome.tabs.query({});
            ...
            func: (sig) => {
              ...
              if (browserIgnores) {
                try {
                  ...
                  const filtered = list.filter((item) => item !== sig);
                  sessionStorage.setItem('catchy-ignore-browser', JSON.stringify(filtered));
                } catch {
                  // Ignore parse errors
                }
              }
      ...
      // Reload the list
      await loadIgnoredErrors();
    } catch (error) {
      console.error('[Catchy Options] Failed to remove ignored error:', error);
    }
  };

  // Clear all ignored errors from all tabs
  const handleClearAllIgnoredErrors = async () => {
    try {
      const tabs = await chrome.tabs.query({});
      ...
      for (const tab of tabs) {
        ...
        try {
          ...
          // Also send message to clear session ignores
          try {
      ...
      console.error('[Catchy Options] Failed to clear all ignored errors:', error);
    }
  };
    ...
    <div className="min-h-screen bg-background">
      <div className="container max-w-4xl mx-auto py-12 px-4">
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-6">
                ...
                {/* Ignore Button Threshold Slider */}
                <div className="pt-4 border-t border-border">
                  <div className="flex justify-between items-center mb-2">
                    <label className="text-sm font-medium">Ignore button threshold</label>
                    <span className="text-sm font-mono text-muted-foreground">
                      {settings.theme.ignoreButtonThreshold || 3} occurrences
                    </span>
                  </div>
                  <input
                    type="range"
                    ...
                    max="10"
                    step="1"
                    value={settings.theme.ignoreButtonThreshold || 3}
                    onChange={(e) => {
                      saveSettings({
                        ...settings,
                        theme: {
                          ...settings.theme,
                          ignoreButtonThreshold: Number(e.target.value),
                        },
                      });
                    }}
                    className="w-full h-2 bg-muted rounded-lg appearance-none cursor-pointer accent-primary"
                  />
                  <p className="text-xs text-muted-foreground mt-1">
                    🔕 Show "Ignore" button after an error appears this many times per tab. Prevents
                    accidental ignores of one-off errors.
                  </p>
                </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Ignored Errors</CardTitle>
              <CardDescription>
              ...
              <div className="space-y-4">
                ...
                {ignoredErrors.length === 0 ? (
                  ...
                  <div className="space-y-2 max-h-96 overflow-y-auto">
                    {ignoredErrors.map((signature, index) => {
                ...
                <div className="pt-4 border-t border-border">
                  <p className="text-xs text-muted-foreground">
                    💡 <strong>Tip:</strong> Ignored errors are stored per browser session. They will be cleared when you close the browser or reload the page.
                  </p>
                </div>
              </div>
            </CardContent>
